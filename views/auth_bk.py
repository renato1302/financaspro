...
def render_auth():
    ...
    with tab_cadastrar:
        with st.form("form_cadastro"):
            ...
            nivel = st.selectbox("Nível de Acesso", ["Somente Leitura", "Consegue Ler e Lançamentos", "Administrador"])
                ...
                if novo_user and nova_senha and novo_email:
                    ...
                    if not df_users.empty and novo_user in df_users['username'].values:
                        ...
                        executar_query("INSERT INTO usuarios VALUES (?, ?, ?, ?)",
                                       (novo_user.strip(), hash_password(nova_senha), novo_email.strip(), nivel))
                        st.success("Usuário cadastrado com sucesso! Volte à aba de Login para acessar.")
                else:
