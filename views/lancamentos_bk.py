...
def render_lancamentos():
    ...
    if df_contas.empty or df_cats.empty:
        st.warning("⚠️ Cadastre suas Contas e a Hierarquia de Categorias nas Configurações.")
        return

    # Painel para Adicionar Novos Lançamentos
    ...
    # Painel para Visualizar, Modificar ou Excluir Lançamentos
    ...
    if not df_trans.empty:
        ...
        with st.expander("✏️ Editar ou 🗑️ Excluir Lançamento Existente"):
            # Cria um label fácil de ler para o usuário selecionar
            df_trans['label'] = df_trans['id'].astype(str) + " | " + df_trans['data'].astype(str) + " | " + df_trans[
                'descricao'] + " | R$ " + df_trans['valor'].astype(str)
            ...
            if trans_sel_label:
                ...
                # Listas Suspensas (Fora do Form para manter a lógica de cascata nativa do Streamlit na Edição)
                grupos = sorted(df_cats['grupo'].unique())
                default_g = row['grupo'] if row['grupo'] in grupos else (grupos[0] if grupos else None)
                edit_g = st.selectbox("Grupo", grupos, index=grupos.index(default_g) if default_g else 0, key="ed_g")

                subgrupos = sorted(df_cats[df_cats['grupo'] == edit_g]['subgrupo'].unique())
                default_sg = row['subgrupo'] if row['subgrupo'] in subgrupos else (subgrupos[0] if subgrupos else None)
                edit_sg = st.selectbox("Subgrupo", subgrupos, index=subgrupos.index(default_sg) if default_sg else 0,
                                       key="ed_sg")

                subcats = sorted(
                    df_cats[(df_cats['grupo'] == edit_g) & (df_cats['subgrupo'] == edit_sg)]['subcategoria'].unique())
                default_sc = row['subcategoria'] if row['subcategoria'] in subcats else (
                    subcats[0] if subcats else None)
                edit_sc = st.selectbox("Sub-Categoria", subcats, index=subcats.index(default_sc) if default_sc else 0,
                                       key="ed_sc")
                ...
                if col_btn1.button("💾 Atualizar Lançamento", type="primary"):
                    if edit_val > 0:
                        ...
                        executar_query("""
                            ...
                            WHERE id=?
                        """, (valor_f, edit_tipo, edit_g, edit_sg, edit_sc, edit_conta, edit_data, edit_desc, trans_id))
